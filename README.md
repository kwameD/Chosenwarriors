...
Visit `#admin` to upload pictures from desktop or mobile, crop admin-managed images, update the home page event highlight, event dates, event times, event details, leadership profiles, and ministry links. Set the Terraform `admin_password` variable before using the admin editor in production:
